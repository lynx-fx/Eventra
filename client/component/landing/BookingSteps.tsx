        ...
        <section className="py-24 relative z-10">
            <div className="container mx-auto px-4 text-center">
                <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-16">
                    Booking made as easy as <span className="text-purple-600 dark:text-purple-500">1-2-3</span>.
                </h2>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
                    {/* Connector Line (Desktop) */}
                    <div className="hidden md:block absolute top-12 left-[16%] right-[16%] h-0.5 -z-10 border-t border-dashed border-border"></div>

                    {steps.map((step, index) => (
                        <div key={index} className="flex flex-col items-center group">
                            <div className="w-24 h-24 bg-card rounded-3xl flex items-center justify-center border border-border shadow-xl mb-6 relative z-10 group-hover:scale-110 transition-transform duration-300">
                                {step.icon}
                                <div className="absolute -top-3 -right-3 w-8 h-8 bg-purple-600 rounded-full flex items-center justify-center font-bold text-white text-sm border-4 border-background shadow-md">
                                    {index + 1}
                                </div>
                            </div>

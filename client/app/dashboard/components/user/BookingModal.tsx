            ...
            {isOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <motion.div
                        ...
                        className="relative w-full max-w-4xl bg-card rounded-[40px] border border-border shadow-2xl overflow-hidden flex flex-col md:flex-row min-h-[500px]"
                    >
                        {/* Left Side: Event Details */}
                        <div className="w-full md:w-2/5 p-10 bg-linear-to-br from-purple-600/10 to-transparent flex flex-col justify-between">
                            <div>
                                <span className="px-4 py-1.5 bg-primary/20 text-primary text-[10px] font-bold uppercase tracking-widest rounded-full border border-primary/20">
                                    Secure Checkout
                                </span>
                                <h3 className="text-3xl font-serif text-foreground mt-6 leading-tight">{event.title}</h3>

                                <div className="space-y-6 mt-10">
                                    <div className="flex items-center gap-4 text-muted-foreground group">
                                        <div className="w-10 h-10 rounded-2xl bg-primary/10 flex items-center justify-center group-hover:text-primary transition-colors">
                                            <Calendar size={20} />
                                        </div>
                                        <div>
                                            <p className="text-[10px] uppercase tracking-widest text-muted-foreground font-bold">Event Date</p>
                            ...
                            <div className="mt-12 p-6 bg-background rounded-3xl border border-border">
                                <p className="text-xs text-muted-foreground mb-2 uppercase tracking-wide">Summary</p>
                        ...
                        <div className="flex-1 p-10 flex flex-col">
                            <div className="flex justify-between items-center mb-10">
                                <div className="flex gap-2">
                                    <div className={`w-2 h-2 rounded-full ${step === 1 ? 'bg-primary' : 'bg-muted-foreground'}`} />
                                    <div className={`w-2 h-2 rounded-full ${step === 2 ? 'bg-primary' : 'bg-muted-foreground'}`} />
                                </div>
                            ...
                            {step === 1 ? (
                                <div className="flex-1 overflow-y-auto custom-scrollbar pr-2">
                                    <h4 className="text-xl font-serif text-foreground mb-6">Choose your experience</h4>
                                        ...
                                        {ticketTypes.map((type) => (
                                            ...
                                            >
                                                <div className="flex justify-between items-start mb-4 relative z-10 pr-6">
                                                    <div>
                                                        <h5 className="font-bold text-lg text-foreground group-hover:text-primary transition-colors">{type.name}</h5>
                                                        <p className="text-xs text-muted-foreground">{type.available} seats remaining</p>
                                                    </div>
                                                    <p className="text-xl font-bold font-mono text-foreground text-right">NPR {type.price.toFixed(2)}</p>
                                                </div>
                                ...
                                <div className="flex-1 flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
                                    <h4 className="text-xl font-serif text-foreground mb-6">Payment Method</h4>
                                    ...
                                    <div className="space-y-4 mb-10">
                                        <div className="p-6 bg-green-600/10 border border-green-500/30 rounded-3xl flex items-center justify-between">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <p className="text-sm font-medium text-foreground">eSewa Mobile Wallet</p>
                                                    <p className="text-xs text-muted-foreground">Pay securely via eSewa</p>
                                                </div>
                                            </div>
                                    ...
                                    <div className="mt-auto space-y-4">
                                        ...
                                        <div className="flex justify-between text-sm">
                                            ...
                                            <span className="text-foreground">NPR 0.00</span>
                                        </div>
                                        ...
                                        <div className="flex gap-4 pt-6">
                                            <button
                                                onClick={() => setStep(1)}
                                                disabled={isLoading}
                                                className="flex-1 py-4 border border-border text-muted-foreground hover:text-foreground hover:bg-background rounded-2xl font-bold transition-all"
                                            >
                                                Back
                                            </button>
                                            <button
